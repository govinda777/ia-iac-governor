...
def validate_sqs():
    ...
    start_time = time.time()
    run_cmd(f"aws sqs send-message --queue-url {queue_url} --message-body 'Hello Floci' --endpoint-url=http://localhost:4566")
    send_latency = (time.time() - start_time) * 1000
    print(f"Send message latency: {send_latency:.2f} ms")

    # Measure receive message latency
    start_time = time.time()
    run_cmd(f"aws sqs receive-message --queue-url {queue_url} --endpoint-url=http://localhost:4566")
    receive_latency = (time.time() - start_time) * 1000
    print(f"Receive message latency: {receive_latency:.2f} ms")

    # Clean up
    run_cmd(f"aws sqs delete-queue --queue-url {queue_url} --endpoint-url=http://localhost:4566")
